...
void encrypt(unsigned char plainText[], unsigned char key[], char file[]){
    ...
    unsigned long long int *shifter = calloc(LEN,sizeof(unsigned long long int));
    ...
    for(i=1; i<plainTextLen; i++){
        pseudo[i] = (unsigned char)(myPseudoRand(pseudo[i-1],salt,shifter[i-1]));
    }
