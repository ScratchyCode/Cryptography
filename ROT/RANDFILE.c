...
unsigned long long int myPseudoRand(unsigned long long int seed, unsigned long long int salt);
...
void encrypt(char file[], unsigned char key[]){
    ...
    unsigned long int seed=0, salt=0, salt1=0, salt2=0, salt3=0;
    ...
    do{
        ...
        if(lenKey % 2 == 0){
            ...
            for(i=0; i<MAXLEN; i++){
                ...
                keyChar = (unsigned char)(myPseudoRand(keyChar,salt));
...
void decrypt(char file[], unsigned char key[]){
    ...
    // generating salt to extract a seed
    unsigned long int seed, salt, salt1=0, salt2=0, salt3=0;
    for(i=0; i<SHA_DIGEST_LENGTH; i++){
        salt1 += (unsigned int)hash1[i];
    }
    for(i=0; i<SHA256_DIGEST_LENGTH; i++){
...
unsigned long long int myPseudoRand(unsigned long long int seed, unsigned long long int salt){
    ...
    if(pseudo % 2 == 0){
        pseudo = rotr(pseudo,salt);
    }else{
        pseudo = rotl(pseudo,salt);
    }
