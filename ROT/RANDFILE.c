...
void encrypt(char file[], unsigned char key[]){
    ...
    unsigned long long int *shifter = calloc(lenFile,sizeof(unsigned long long int));
    checkPtr(hash1);
    checkPtr(hash2);
    checkPtr(hash3);
    checkPtr(encrypted);
    ...
    unsigned long long int seed=0, salt=0, salt1=0, salt2=0, salt3=0;
    ...
    do{
        ...
        if(seed % 2 == 0){
            ...
            for(i=0; i<MAXLEN; i++){
                ...
                keyChar = (unsigned char)(myPseudoRand(keyChar,salt,shifter[i]));
...
void decrypt(char file[], unsigned char key[]){
    ...
    unsigned long long int *shifter = calloc(lenEncrypted,sizeof(unsigned long long int));
